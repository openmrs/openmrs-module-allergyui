                    ...
                    <ul ng-show="allergenType == '${ typeName }'">
                        ...
                        <li>
                            <% if (allergen.id == otherNonCodedConcept.id) { %>
                             <input id="allergen-${ typeName }" type="radio" name="codedAllergen" value="${allergen.id}" ng-click="selectOtherAllergenRadioInput()" class="coded_allergens" ng-model="allergen"
                                    ${(allergy.allergen != null && allergen == allergy.allergen.codedAllergen) ? "checked=checked" : ""}/>
                            <% } else { %>
                                <input id="allergen-${allergen.id}" type="radio" name="codedAllergen" value="${allergen.id}" ng-click="controlOtherAllergenInputVisibility()" class="coded_allergens" ng-model="allergen"
                                    ${(allergy.allergen != null && allergen == allergy.allergen.codedAllergen) ? "checked=checked" : ""}/>
                            <% } %>
            ...
            <% reactionConcepts.each { reaction -> %>
                <li>
                    ...
                    <% if (reaction.id == otherNonCodedConcept.id) { %>
                    	 <input 
                    	 ng-model="reaction${reaction.id}" type="checkbox" id="reaction-${reaction.id}"
                            class="allergy-reaction"
                            name="allergyReactionConcepts" value="${reaction.id}"
                            ng-click="controlOtherReactionInputVisibility(${reaction.id})" ng-init="reaction${reaction.id} = ${allergyReactionConcepts.contains(reaction)}"/>
                        <label for="reaction-${reaction.id}"
                            ng-click="controlOtherReactionInputVisibility(${reaction.id})">${ui.format(reaction)}</label>
                        <input type="text" maxlength="255" name="reactionNonCoded"
                            ng-init="initOtherReactionInputVisibility(reaction${reaction.id})" ng-focus="otherReactionFocus(${reaction.id})"
                            <% if (allergy.reactionNonCoded) { %> value="${allergy.reactionNonCoded}" <% } %>/>
                    <% } else { %>
                        <input ng-model="reaction${reaction.id}" type="checkbox" id="reaction-${reaction.id}" 
                        class="allergy-reaction" name="allergyReactionConcepts" value="${reaction.id}" ng-init="reaction${reaction.id} = ${allergyReactionConcepts.contains(reaction)}"/>
